...
const UpdateCarPage = () => {
  ...
  useEffect(() => {
    const fetchCarData = async () => {
      try {
        ...
        setCarData(carData); // Adjust this based on your API response structure
        ...
        setValue("available_at", dayjs(carData.available_at));
        setValue("transmission", carData.transmission);
        setValue("available", carData.available);
        setValue("type", carData.type);
        setValue("year", carData.year);

        // Handle options and specs fields if they are arrays
        setOptionsFields(carData.options || []);
        setSpecsFields(carData.specs || []);

        // Handle image preview if image URL is available in carData
        if (carData.image) {
    ...
    fetchCarData();
  }, [id, setValue, setOptionsFields, setSpecsFields, setImage]);
  ...
  const handleRemoveOptionsFields = (index) => {
    ...
    setOptionsFields(newOptionsFields);
    // Hapus nilai pada form menggunakan setValue
    setValue("options", newOptionsFields);
  };
            ...
            <Stack spacing={3}>
              ...
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <Controller name="available_at" control={control} render={({ field }) => <DatePicker {...field} renderInput={(props) => <TextField {...props} />} />} />
              ...
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Controller
                  name="transmission"
                  control={control}
                  defaultValue={carData ? carData.transmission : ""} // Set default value here
                  render={({ field }) => (
              ...
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Controller
                  name="available"
                  control={control}
                  defaultValue={carData ? carData.available : ""}
                  render={({ field }) => (
                    <RadioGroup row {...field}>
              ...
              <Box sx={{ display: "flex", alignItems: "center" }}>
                ...
                <Stack spacing={1}>
                  <Input type="file" {...register("image")} onChange={handleChangeImage} />
